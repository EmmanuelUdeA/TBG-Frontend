...
const Store = () => {
    ...
    return (
        <div className="flex w-screen h-auto">
            <div className="flex flex-row flex-wrap w-full h-auto justify-start items-start">
                ...
                <ul className="flex flex-row w-3/4 h-screen justify-center items-start my-20 flex-wrap">
                    {products && products.map((p: any, i: any) => {
                        if (i < 8) return (
                            <li key={i} className="flex flex-col w-1/6 h-auto justify-center items-center mx-8 mb-10">
                                <Link href={"/store/product/" + p.id} className="w-full h-auto flex flex-col justify-center items-center ">
                                    {p.front_img ? <img src={p.front_img} alt={p.name} className="flex w-full h-96 justify-center items-center" /> :
