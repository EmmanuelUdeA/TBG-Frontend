const Help = () => {
    return (
        <div className="flex flex-col justify-center items-center w-full h-screen text-center p-5 mt-28">
            <h2 className="text-2xl font-bold mb-5 bg-black text-yellow-50 w-full p-5">Help</h2>
            <div className="flex flex-col justify-center items-center w-full">
                <h3 className="font-semibold text-4xl mb-4 h-10 w-full">Frequent Question</h3>
                <p className="text-center w-1/2 h-auto my-3 ">Aquí puedes escribir sobre las preguntas frecuentes Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem perspiciatis beatae quos repudiandae officia. Molestias voluptatum provident quod. Tenetur at tempore numquam libero eius nostrum, adipisci labore eveniet dolore recusandae?
                    Iste minus, molestias repellendus rerum iusto necessitatibus eum expedita, nostrum modi, porro fugiat. Deserunt iusto, asperiores ullam distinctio veniam voluptatibus ut modi saepe impedit eos. Exercitationem dolore voluptatem error cumque!.</p>

                <h3 className="font-semibold text-4xl mb-4 h-10 w-full">Refunds and Warranties</h3>
                <p className="text-center w-1/2 h-auto my-3 ">Aquí puedes escribir sobre los reembolsos y garantias. Lorem ipsum dolor sit amet consectetur adipisicing elit. Nihil laborum architecto dolore molestiae dolor ipsam debitis inventore voluptate aperiam voluptates minus illum nesciunt officia, rerum soluta harum quidem, saepe voluptas.
                    Iusto eveniet soluta dolore, placeat excepturi consequuntur neque totam deleniti at illum error commodi a! Natus soluta voluptatem sint, eum asperiores, at dolorem consectetur aliquam labore nisi quos quis molestiae!</p>

                <h3 className="font-semibold text-4xl mb-4 h-10 w-full">Shipping</h3>
                <p className="text-center w-1/2 h-auto my-3 ">Aquí puedes escribir sobre el envio. Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati excepturi asperiores nam, modi cupiditate impedit laboriosam error assumenda accusamus dignissimos! Voluptatum repudiandae cupiditate non vitae ducimus ut assumenda fugiat exercitationem.
                    Adipisci laboriosam dolor perferendis, ut itaque sint natus blanditiis magnam eaque libero reiciendis facere qui exercitationem, voluptatem est distinctio quidem consequuntur, eos iusto earum officia vel ratione pariatur. Et, quisquam.</p>
            </div>
        </div>
    )
}

export default Help;